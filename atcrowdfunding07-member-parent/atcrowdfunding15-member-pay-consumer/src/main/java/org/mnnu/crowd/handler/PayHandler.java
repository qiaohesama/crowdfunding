...
import org.mnnu.crowd.config.PayProperties;
...
public class PayHandler {
    ...
    /**
     * 用户发起付款请求后，系统将订单信息收集，然后调用支付宝的接口向支付宝发送请求
     *
     * @param orderVO
     * @param session
     * @return
    ...
    public String generateOrder(OrderVO orderVO,
                                HttpSession session) throws AlipayApiException {
        // 1.获取orderProjectVO并和orderVO组装到一起
        ...
        String orderNum = dateTime + uuid;
        orderVO.setOrderNum(orderNum);
        // 3.计算总金额并设置
        int orderAmount = orderProjectVO.getReturnCount() * orderProjectVO.getSupportPrice() + orderProjectVO.getFreight();
        orderVO.setOrderAmount((double) orderAmount);

        session.setAttribute(CrowdConstant.ATTR_NAME_ORDER_VO, orderVO);
        // 4. 调用方法给支付宝发送请求
        return sendPayRequest(orderNum,
                (double) orderAmount,
     ...
     */
    private String sendPayRequest(String outTradeNo,
                                  Double totalAmount,
                                  String subject,
...
// 获取支付宝 GET 过来反馈信息
        ...
        if (signVerified) {
            ...
            ResultEntity<String> resultEntity = sqlRemoteService.saveOrderPORemote(orderVO);
            if (ResultEntity.SUCCESS.equals(resultEntity.getOperationResult())) {
                // 这里需要测试
                return "redirect:http://localhost/auth/my/crowd";
            } else {
                log.warn("用户订单数据保存失败，订单号为:" + orderVO.getOrderNum());
                throw new RuntimeException("订单保存失败，请联系管理员退款");
            }
        } else {
            // 页面显示信息： 验签失败
            log.error("验签失败");
